...
class OtaHelper {
  ...
  bool Start
          ...
          bool
          BldUpdtTask(const datatypes::MPUUpdateTaskBuildingReq &req, datatypes::MPUUpdateTaskBuildingResp &resp,
                      std::uint32_t timeout_ms);

  bool StwrTrsmtPrgs(std::uint16_t task_id, datatypes::MPUTransmitProgressResp &resp, std::uint32_t timeout_ms);

  bool StopUpdt(const datatypes::MPUUpdateStopReq &req, datatypes::MPUUpdateStopReq &resp, std::uint32_t timeout_ms);

  bool StartInst(std::uint16_t task_id, datatypes::MPUInstallStartResp &resp, std::uint32_t timeout_ms);

  bool ReqInstResult(std::uint16_t task_id, datatypes::MPUInstallResultResp &resp, std::uint32_t timeout_ms);

  bool SwitchABBank(std::uint16_t task_id, datatypes::MPUStatusSwitchResp &resp, std::uint32_t timeout_ms);

  bool GetUpdtTaskStatus(std::uint16_t task_id, datatypes::MPUGetStatusResp &resp, std::uint32_t timeout_ms);

  bool ReqSwitchResult(std::uint16_t task_id, datatypes::MPUSwitchResultResp &resp, std::uint32_t timeout_ms);
